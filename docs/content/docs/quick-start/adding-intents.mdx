...
The next step is to actually define our intents. I'll first show you the code for one such intent:
...
Powered by Structured Output, it is guarenteed the AI's output will match your schema.
...
For both `onIntent` and `onCleanup`, they take a callback with two parameters: `intentCall` and `input`.
